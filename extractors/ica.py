...
from config import ICA_HIGH_HZ, ICA_LOW_HZ
...
def ica_algorithm(frames, FS):
    ...
    BVP_I = S[MaxComp, :]
    low, high = band_edges(FS, ICA_LOW_HZ, ICA_HIGH_HZ)
...
def ica(X, Nsources, Wprev=0):
    nRows = X.shape[0]
    nCols = X.shape[1]
    # Nenhum dos dois avisos é alcançável com as 3 ROIs do contrato (X sai
    # 3xN, N >> 3); ficam como logger, não print, para não sujar o stdout do
    # servidor caso a forma da entrada mude um dia.
    if nRows > nCols:
        logger.warning(
            "The number of rows cannot be greater than the number of columns. "
            "Please transpose input.")

    if Nsources > min(nRows, nCols):
        Nsources = min(nRows, nCols)
        logger.warning(
            "The number of sources cannot exceed the number of observation "
            "channels; reduced to %s.", Nsources)
